...
def get_hoverdata(activity_dict,reversed=False):
    ...
    customdata = [
            [activity_dict['name'],                                             # Activity name
             insert_line_breaks(activity_dict['description'].replace('\n','<br>'), max_length=80),   # Description
...
def add_activity_line_to_map(fig, activity_dict, line_color='blue', line_width=4.5, row=None, col=None):
    ...
    hovertemplate, customdata = get_hoverdata(activity_dict)
...
def plot_collection_combined(collection_name, client, reload=False):
    ...
    colors = [f"rgb({r},{g},{b})" for r, g, b in np.random.randint(0, 255, (len(activity_ids), 3))]
    ...
    for idx, (activity_id, reversed) in enumerate(zip(activity_ids, activity_reversed)):
        ...
        fig = add_activity_line_to_map(fig, activity_dict, row=ROW_MAP, col=1, line_color=colors[idx])
    ...
    x0 = 0
    for idx, (activity_id, reversed) in enumerate(zip(activity_ids, activity_reversed)):
        activity_dict = sp.import_activity(activity_id, client, reload=reload, reversed=reversed)
        ...
        fig.add_trace(
            go.Scatter(
                ...
                customdata=customdata * len(activity_dict['stream_dict']['distance'][::N]),
                hovertemplate=hovertemplate,
                name=activity_dict['name'],
                fill='tozeroy',
            ),
            row=ROW_ALTITUDE, col=1
        )
    ...
    table_header = ["Name", "Date",
                    #"Time",
                    "Distance (km)", "Moving Time (h)", "Elevation Gain (m)", "Max Elevation (m)", "Average Heart Rate (bpm)", "Max Heart Rate (bpm)", "Strava Link"]
    table_cells = []
    ...
    for activity_id in activity_ids:
        ...
        date = datetime.fromisoformat(str(activity_dict['start_date'])).strftime("%Y-%m-%d")
        time = datetime.fromisoformat(str(activity_dict['start_date'])).strftime("%H:%M")
    ...
    fig.add_trace(
        go.Table(
            header=dict(values=table_header, fill_color='paleturquoise', align='left', font_size=12),
            cells=dict(values=list(zip(*table_cells)), fill_color='lavender', align='left', font_size=11)
        ),
        row=ROW_TABLE, col=1
    )
...
def main():
    ...
    parser.add_argument('collection', choices=possible_collections, 
    ...
    if collection == 'all':
        collections = ['norway-turkey','berlin-tarifa','hue-hcmc_2016','taiwan_2017','hue-hcmc_2016','yokohama-fukuoka_2019','bavarian-alp-traverse']
    else:
